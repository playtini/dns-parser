...
func init() {
	...
	flag.IntVar(&concurrency, "concurrency", 5000, "Internal buffer")
	flag.IntVar(&packetsPerSecond, "pps", 120, "Send up to PPS DNS queries per second")
	flag.StringVar(&retryTime, "retry", "1s", "Resend unanswered query after RETRY")
	flag.BoolVar(&verbose, "v", false, "Verbose logging")
	flag.Uint64Var(&dnsType, "dns-type", dnsTypeA, "dnsTypeA = 1, dnsTypeNS = 2, dnsTypeMX = 15, dnsTypeTXT = 16, dnsTypeAAAA  = 28")
}
...
func main() {
	...
	go doSend(c, tryResolving)
	go doReceive(c, resolved)
	...
	for done == false || len(m) > 0 {
		...
		case da := <-resolved:
			if m[da.id] != nil {
				...
				sort.Sort(sort.StringSlice(da.values))

				// without trailing dot
				domain := dr.domain[:len(dr.domain)-1]
				fmt.Printf("%s\t%s\n", domain, strings.Join(da.values, " "))
...
func doTimeouter(timeoutRegister <-chan *domainRecord, timeoutExpired chan<- *domainRecord) {
	for {
		dr := <-timeoutRegister
...
func doSend(c net.Conn, tryResolving <-chan *domainRecord) {
	for {
		dr := <-tryResolving
...
func doReceive(c net.Conn, resolved chan<- *domainAnswer) {
	buf := make([]byte, 4096)
	for {
		n, err := c.Read(buf)
		if err != nil {
		...
		domain, id, values := unpackDns(buf[:n], t)
		resolved <- &domainAnswer{id, domain, values}
	}
}
