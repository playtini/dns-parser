...
func convertRR_Ns(records []dnsRR) (result []string) {
	result = make([]string, len(records))
	for i, rr := range records {
		result[i] = rr.(*dnsRR_NS).Ns
	}

	return
}

func convertRR_Mx(records []dnsRR) (result []string) {
	result = make([]string, len(records))
	for i, rr := range records {
		result[i] = rr.(*dnsRR_MX).Mx
	}

	return
}

func convertRR_Txt(records []dnsRR) (result []string) {
	result = make([]string, len(records))
	for i, rr := range records {
		result[i] = rr.(*dnsRR_TXT).Txt
	}
